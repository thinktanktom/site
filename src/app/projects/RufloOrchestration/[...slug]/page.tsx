...
import { getAllDocSlugs, getDocBySlug, getAdjacentPages } from '@lib/ruflo-orchestration-docs'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!doc) return {}
  const title = `${doc.title} — AI Agent Orchestration`
  const description = `AI Agent-Orchestration Infrastructure documentation: ${doc.title}`
  return {
    title,
    description,
    alternates: { canonical: `/projects/RufloOrchestration/${params.slug.join('/')}` },
    openGraph: {
      title,
      description,
      type: 'article',
      url: `https://thinktanktom.com/projects/RufloOrchestration/${params.slug.join('/')}`,
...
export default function RufloOrchestrationDocPage({ params }: Props) {
  const doc = getDocBySlug(params.slug)
  if (!doc) notFound()
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Projects', item: 'https://thinktanktom.com/projects' },
      {
        ...
        item: 'https://thinktanktom.com/projects/RufloOrchestration',
      },
      {
        '@type': 'ListItem',
        position: 4,
        name: doc.title,
        item: `https://thinktanktom.com/projects/RufloOrchestration/${params.slug.join('/')}`,
      },
    ],
  }
    ...
    <>
      ...
      {!(params.slug.length === 1 && params.slug[0] === 'introduction') && (
        <nav className="mt-16 pt-8 border-t border-border flex items-center justify-between gap-4">
          {prev ? (
            <Link
              href={`/projects/RufloOrchestration/${prev.slug.join('/')}`}
          ...
          {next ? (
            <Link
              href={`/projects/RufloOrchestration/${next.slug.join('/')}`}
