...
import { getAllDocSlugs, getDocBySlug, getAdjacentPages } from '@lib/lockbox-docs'
...
export default function LockBoxDocPage({ params }: Props) {
  const doc = getDocBySlug(params.slug)
  if (!doc) notFound()
    ...
    <>
      ...
      {!(params.slug.length === 1 && params.slug[0] === 'introduction') && (
        <nav className="mt-16 pt-8 border-t border-border flex items-center justify-between gap-4">
          {prev ? (
            <Link
              href={`/projects/LockBox/${prev.slug.join('/')}`}
          ...
          {next ? (
            <Link
              href={`/projects/LockBox/${next.slug.join('/')}`}
