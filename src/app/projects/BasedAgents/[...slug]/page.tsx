...
import { getAllDocSlugs, getDocBySlug, getAdjacentPages } from '@lib/based-agents-docs'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!doc) return {}
  const title = `${doc.title} — Based Agents`
  const description = `Based Agents smart contract suite documentation: ${doc.title}`
  return {
    title,
    description,
    openGraph: {
      ...
      description,
      type: 'article',
      url: `https://thinktanktom.com/projects/BasedAgents/${params.slug.join('/')}`,
...
export default function BasedAgentsDocPage({ params }: Props) {
  const doc = getDocBySlug(params.slug)
  if (!doc) notFound()
    ...
    <>
      ...
      {!(params.slug.length === 1 && params.slug[0] === 'introduction') && (
        <nav className="mt-16 pt-8 border-t border-border flex items-center justify-between gap-4">
          {prev ? (
            <Link
              href={`/projects/BasedAgents/${prev.slug.join('/')}`}
          ...
          {next ? (
            <Link
              href={`/projects/BasedAgents/${next.slug.join('/')}`}
