...
date: "2026-05-12"
excerpt: "A complete record of getting Ruflo running on a DigitalOcean VM: the npm path that didn't work, five separate issues that each needed diagnosing, and a live chat UI and remote MCP connection the client can use from their own machine."
tags: ["ai", "devops", "self-hosted", "claude", "docker"]
aiGenerated: true
---

This post is a full narrative of building a self-hosted AI orchestration environment for a client, including every dead end and the diagnosis that got past it.

The end result was two capabilities running on a DigitalOcean VM (Ubuntu, 2 vCPU / 4 GB RAM). First, a live web-based chat UI, backed by Claude models via Ruflo's MCP bridge, secured with HTTPS and basic auth. Second, a remote MCP connection so the client's Claude Code desktop can reach the VM's MCP bridge directly, giving agentic access to the project codebase without SSH.
...
For the subdomain, I used DuckDNS rather than a paid domain. Caddy handles the Let's Encrypt certificate automatically once the subdomain resolves, so no further TLS work was needed.

### Getting the Stack Up

Three issues needed resolving before the stack was cleanly operational.

**Issue: Claude models not appearing in the UI.** Once the UI was accessible, the model selector was empty. The cause: the mcp-bridge does not auto-discover models from API providers. Models have to be explicitly registered in a `KNOWN_MODELS` array inside the bridge's source code at `src/ruvocal/mcp-bridge/index.js`:
...
This isn't mentioned prominently in any docs. The UI loads fine without it; it just shows no models to select. ([commit `dbc60d9`](https://github.com/thinktanktom/ruflo/commit/dbc60d9c68bd25f63a74846c56b7ccce342c4da0))
...
Docker Compose's environment variable resolution order runs shell environment first, then the `.env` file, then `docker-compose.yml` defaults. Stale shell exports will always win over `.env` values, silently. ([commit `068a072`](https://github.com/thinktanktom/ruflo/commit/068a0720159a1e1dbfa92e17a9632d2516bcefc3))
...
With the stack running, the next step was giving the client's Claude Code desktop direct access to the MCP bridge on the VM, without SSH and without a VPN.
...
Version control is the minimum viable safety net for giving an agent write access to a codebase.

---

## Security Posture

Most self-hosted AI writeups skip this section. This one doesn't.

### How secure is a bearer token as a handover mechanism

It's acceptable but not great. A static shared secret has two structural weaknesses: it never expires unless rotated manually, and it grants the same level of access every time it's used, from anywhere, forever. There's no per-session audit trail and no way to revoke it without changing it for everyone simultaneously.
...
The honest answer is that it's large. The mcp-bridge exposes 204+ tools including `devtools`, `agents`, `memory`, `intelligence`, and `terminal_execute`. Anyone with the token can execute arbitrary shell commands on the VM as the Docker process user, read and write anything in the mounted project workspace, read the container's environment variables (which include the Anthropic and OpenRouter API keys), drain those API keys by sending requests through the bridge, and exfiltrate the entire codebase.

The API key exposure is probably the most immediately damaging item on that list. The right mitigation is to scope down what the remote Claude Code connection can actually do. The client doesn't need terminal execution or agent spawning; they need file access and the workflow trigger tools. Separate the client-facing route and restrict it to safe tool groups:
...
Two separate tokens, one for the internal chat UI and one for the remote client, means they can be revoked independently.
...
3. **Use two separate tokens,** one for the chat UI and one for the remote client, so they can be revoked independently.
4. **Scope the client-facing MCP route to safe tool groups.** No `terminal_execute`, no `agents`.
5. **Enable Caddy access logging** so there's an audit trail of every request that hits the bridge.
